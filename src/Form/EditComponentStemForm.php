...
class EditComponentStemForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $componentstemuri = NULL) {
    ...
    if (!empty($componentstem_image)) {
      $image_view_url = '';
      if ($image_type === 'url') {
        $image_view_url = $componentstem_image;
      }
      elseif (!empty($modUri)) {
      ...
      if ($image_view_url !== '') {
        $form['componentstem_information']['componentstem_image_preview'] = [
          '#type' => 'markup',
    ...
    if (!empty($componentstem_webdocument)) {
      $webdoc_view_url = '';
      if ($webdocument_type === 'url') {
        $webdoc_view_url = $componentstem_webdocument;
      }
      elseif (!empty($modUri)) {
      ...
      if ($webdoc_view_url !== '') {
        $form['componentstem_information']['componentstem_webdocument_preview'] = [
          '#type' => 'markup',
          '#markup' => Markup::create(
            '<div class="mt-2">'
