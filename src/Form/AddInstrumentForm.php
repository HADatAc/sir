...
class AddInstrumentForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    try{
      ...
      $namespace_abbreviation = $config->get("repository_abbreviation");
      $namespace_url = $config->get("namespace_url");
      ...
      $data = [];
       ...
       $datap = '{"uri":"'.$namespace_url.'kb/'.$namespace_abbreviation.'/Instrument'.$iid.'","typeUri":"http://hadatac.org/ont/vstoi#Questionnaire","hascoTypeUri":"http://hadatac.org/ont/vstoi#Instrument","label":"'.$form_state->getValue('instrument_name').'","comment":"'.$form_state->getValue('instrument_description').'","hasShortName":"'.$form_state->getValue('instrument_abbreviation').'","hasSIRMaintainerEmail":"'.$uemail.'"}';
  
      $dataE = rawurlencode($datap);

      $newInstrument = $this->addInstrument($api_url,"/sirapi/api/instrument/create/".$dataE,$data);
   
      $instrument_url = 'http://hadatac.org/kb/test/Instrument' . $iid;
      $url_object = Url::fromRoute('sir.itens_of_instruments', ['instrument' => rawurlencode($instrument_url)]);
