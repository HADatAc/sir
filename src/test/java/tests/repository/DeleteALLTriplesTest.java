...
public class DeleteALLTriplesTest extends BaseRep {
    ...
    void testDeleteALLTriplesAndCheckValues() throws InterruptedException {
        ...
        System.out.println("Waiting for all rows to Load...");
        ...
        while (!allDataLoaded && attempt < 30) {
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                if (cells.size() >= 7) {
                    ...
                    boolean triplesValid = triples.isEmpty();
        ...
        Assertions.assertTrue(allDataLoaded, "Not all rows have no Triples.");
        System.out.println("✔ All valid rows have No Triples Anymore.");
    }
