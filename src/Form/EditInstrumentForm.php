...
class EditInstrumentForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $instrumenturi = NULL) {
    ...
    if ($socialEnabled) {
      $makerUri = '';
      if (isset($this->getInstrument()->hasMakerUri) && $this->getInstrument()->hasMakerUri !== null) {
        $makerUri = $api->parseObjectResponse($api->getUri($this->getInstrument()->hasMakerUri), 'getUri');
      }
    ...
    $form['instrument_information']['instrument_parent_wrapper']['instrument_fidelity'] = [
      '#type' => 'select',
    ...
    $form['instrument_information']['instrument_parent_wrapper']['instrument_anatomy'] = [
      '#type' => 'textarea',
